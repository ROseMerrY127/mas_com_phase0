...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("Phase0 PRM800K config must be a YAML mapping")
    return data
...
async def run_phase0_prm800k(args: argparse.Namespace) -> Path:
    load_dotenv()
    ...
    run_dir = output_root / f"phase0_PRM800K_{timestamp}"
    ...
    if replay_mode:
        ...
        data_path = Path(args.data_path or config.get("data_path", "PRM800K/phase1_train.jsonl"))
    ...
    try:
        for example in tqdm(examples, total=len(examples), desc=f"Phase0 PRM800K {split}", unit="example"):
            try:
                if replay_mode:
                    row = example
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Phase0 PRM800K full-forward MAS experiment.")
...
def main() -> int:
    ...
    try:
        run_dir = asyncio.run(run_phase0_prm800k(args))
    except Exception as exc:  # noqa: BLE001 - CLI should print a concise actionable error.
        print(f"Phase0 PRM800K failed: {exc}", file=sys.stderr)
        return 1
    print(f"Phase0 PRM800K outputs written to: {run_dir}")
    return 0
